...
LastEditTime: 2023-03-13 21:43:17
...
def Plot_static(csv_file):
    ...
    if data_frame.shape[1] <= 3:
        ...
        accel_x_record = data_list[:,1]
        accel_y_record = data_list[:,2]
        accel_z_record = data_list[:,3]

        roll_record = data_list[:,4]
        pitch_record = data_list[:,5]
        yaw_record = data_list[:,6]
        ...
        ax2.set_title("Euler Angles",fontsize=15)
        plt.ylim([-180, 180])
        plt.xlim(time_sequence[0],time_sequence[-1])
        plt.xticks(fontsize = 10)
        plt.yticks(fontsize = 10)
...
csv_file_name = '2023-03-11 22:52:11:76_imu_data_exp_1_OK_2.csv'
